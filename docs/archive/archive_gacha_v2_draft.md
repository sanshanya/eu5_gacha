...
✅ **采用方案**:
- **种子混合**: `total_rolls + pity_count + treasury`
- **数学法取模**: `O(1)` 复杂度，无循环
...
change_variable = { name = gacha_rand add = treasury }  # 动态组件
...
**问题**: 如果抽卡消耗是偶数 (100金)，treasury 奇偶性永不改变 → 玩家永远歪或永不歪。

### 5.2 正确实现

✅ **修复方案**: 混合 `treasury + total_rolls`
...
**验证表格**:
| 抽数 | treasury | total_rolls | 种子和 | mod 2 | 结果 |
|------|----------|------------|--------|-------|------|
| 1 | 1001 | 1 | 1002 | 0 | UP |
| 2 | 901 | 2 | 903 | 1 | 歪 |
| 3 | 801 | 3 | 804 | 0 | UP |
...
gacha_standard_5star_pool = {
    ...
    # 未来扩展: 使用 (treasury + total_rolls) mod N
