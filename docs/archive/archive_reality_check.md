...
文档第 5.1 节声称 treasury 单独使用会导致奇偶锁定：

> 如果抽卡消耗是偶数，treasury 奇偶性永不改变

**现实**:
- ⚠️ 实际代码**只用 `total_rolls` 做 50/50**，根本没用 treasury
- ⚠️ 文档描述的是一个**从未存在的架构**
...
2. **50/50 可预测性**  
   ...
   - 问题: 玩家可以计数  
   - **可行方案**: 混合 treasury: `(total_rolls + treasury) % 2`
