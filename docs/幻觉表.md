...
> **Purpose**: 记录从CK3/EU4/AI迁移到EU5时踩过的坑  
> **Official Reference**: [Tinto Talks #85 - Modding](https://forum.paradoxplaza.com/forum/developer-diary/tinto-talks-85-22nd-of-october-modding.1864004/)  
...
EU5 的官方 Variable 文档里写得很清楚：

Variable type "regular" … Event target: var:

也就是说：

regular 变量的 Event Target 前缀就是 var:

global 变量是 global_var:
...
从语法来看，在 EU5 是正宗合法写法，不是 EU4/CK3 遗留幻觉。
...
### 6.1 `add_adm` / `add_dip` / `add_mil` 必须在 Character Scope 内调用

**幻觉：**

在 Country Scope 下直接调用 `add_adm = 5` 来给角色加能力值。

```paradox
# ❌ 错误：在 Country Scope 下调用
gacha_xinhai_events.11 = {
    type = country_event
    option = {
        add_adm = 5  # 这里 root 是 Country，不是 Character！
    }
}
```
...
EU5 的角色能力值效果 **只能在 Character Scope 内使用**：

| Effect | 功能 | Scope 要求 |
|:---|:---|:---:|
| `add_adm` | 增加行政能力 | Character |
| `add_dip` | 增加外交能力 | Character |
| `add_mil` | 增加军事能力 | Character |
| `add_fertility` | 增加生育率 | Character |
| `add_artist_skill` | 增加艺术技能 | Character |

**正确写法：**

```paradox
# ✅ 正确：先切换到 Character Scope
gacha_xinhai_events.11 = {
    type = country_event
    option = {
        scope:gacha_event_target_char = {
            add_adm = 5  # 现在在 Character Scope 内
            add_dip = 3
            add_mil = 2
        }
    }
...
或者在 scripted_effect 中：

```paradox
# ✅ 正确：确保 this 是 Character
gacha_upgrade_character_stats = {
    # 假设调用时 this = Character
    add_adm = 5
    add_dip = 3  
    add_mil = 2
}
...
# 调用时
scope:my_character = {
    gacha_upgrade_character_stats = yes
}
```

**完整的 Character Scope 效果列表：**
...
add_mil                      - 增加军事能力
add_fertility                - 增加生育率
add_artist_skill             - 增加艺术技能
add_trait                    - 添加特质
remove_trait                 - 移除特质
add_character_modifier       - 添加角色修正
remove_character_modifier    - 移除角色修正
change_character_culture     - 改变角色文化
change_character_religion    - 改变角色宗教
change_character_estate      - 改变角色阶层
marry_character              - 结婚
divorce_character            - 离婚
adopt_character              - 收养
move_country                 - 转移到目标国家
set_first_name               - 设置名字
set_nickname                 - 设置绰号
make_saint                   - 封圣
```
...
凡是操作"角色本身属性"的效果，必须确保当前 Scope 是 Character。在 Country Event 中需要通过 `scope:xxx = { ... }` 或 `random_character = { ... }` 等方式先切入角色 Scope。
