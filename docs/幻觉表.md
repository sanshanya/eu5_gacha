...
但是铁证：“懒加载/卫兵模式”是标准写法
你之前担心 gacha_ensure_state_initialized 这种“先检查有没有，没有再初始化”的写法是否多余。
请看 in_game/common/scripted_effects/wsg_spawning_effect.txt 中的 add_wsg_portrait_count：
code
Paradox
# 引用自 wsg_spawning_effect.txt
add_wsg_portrait_count = {
    # 1. 卫兵检查：如果全局变量不存在，先初始化为 0
    if = {
        limit = {
            NOT = {
                exists = global_var:wsg_portrait_count
            }
        }
        set_global_variable = {
            name = wsg_portrait_count
            value = 0
        }
    }
    # 2. 然后再执行加法
...
也能跑，是因为 Jomini 那边对“裸变量名”做了简化：
在很多上下文里，value = foo 会被解释成“把当前 Scope 上的变量 foo 当成数值/Script Value 用”。

区别在于：

var:foo：强制走变量 Event Target 机制，如果变量没被 set，就会直接报错：

Failed to fetch variable for 'foo' due to not being set

foo（裸变量名）：在变量没被 set 时，通常会被当成“0 / false”对待（不抛错，只是当不存在 → 0）

所以你现在的状态是：

把 var: 全砍掉 → 引擎不再严格检查“变量是否真的存在” → 那些“没初始化就被读”的地方不再报错，而是当 0 用了。

结论：
var: 是对的，报错暴露的是“有地方在变量没初始化时读了它”，而不是 var: 语法错。
